...
def schemas(
    *,
    ...
    """Return a ``Schemas`` namespace for CDF.
    ...
    Accepts the same schema-affecting kwargs as ``cdf.load_tracking``.
    ...
    layout, include_game_id
        Match the same-named kwargs on ``cdf.load_tracking``.
    engine : {"polars", "pyspark", "arrow", "arrow[spark]"}, default "polars"
    ...
    >>> tracking_schema = cdf.schemas(layout="long", engine="arrow[spark]").tracking_spark
    >>> matches_df.mapInArrow(parse_cdf_match_udf, schema=tracking_schema)
    """
    from fastforward._fastforward import cdf as _m
