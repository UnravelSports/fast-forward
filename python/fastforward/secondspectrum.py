...
def schemas(
    *,
    ...
    """Return a ``Schemas`` namespace for SecondSpectrum.
    ...
    Accepts the same schema-affecting kwargs as ``secondspectrum.load_tracking``.
    ``exclude_missing_ball_frames`` is intentionally omitted: it filters rows
    but doesn't change any column's shape.

    Parameters
    ----------
    layout, include_game_id
        Match the same-named kwargs on ``secondspectrum.load_tracking``.
    engine : {"polars", "pyspark", "arrow", "arrow[spark]"}, default "polars"
    ...
    >>> tracking_schema = secondspectrum.schemas(layout="long", engine="arrow[spark]").tracking_spark
    >>> matches_df.mapInArrow(parse_secondspectrum_match_udf, schema=tracking_schema)
    """
    from fastforward._fastforward import secondspectrum as _m
