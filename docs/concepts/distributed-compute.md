...
The example is provider-agnostic. `{provider}` stands for any [single-file provider](#per-provider-input-shape). Multi-file providers (`hawkeye`, `signality`) do not fit this shape; see [Multi-file providers](#multi-file-providers).
...
| `hawkeye`        | `ball_data`, `player_data`, `meta_data`                | yes (per period + minute) | `period=`, `minute=`, see [Multi-file providers](#multi-file-providers) |
...
| `signality`      | `meta_data`, `raw_data_feeds`, `venue_information`     | yes (per period)          | `period=`, see [Multi-file providers](#multi-file-providers) |
...
See [TrackingDataset - Per-engine schema](dataset.md#per-engine-schema).
...
- **Per-match rows for `hawkeye` or `signality`.** Both are multi-file: build the upstream frame with one row per slice and forward `period=` (and `minute=` for HawkEye) from the row. See [Multi-file providers](#multi-file-providers).
