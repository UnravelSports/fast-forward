...
from kloppy.domain import DatasetFlag, PositionType, TrackingDataset
...
class TestCDFSerializer:
    ...
    def test_sportec_flags_in_output(self, dataset_sportec, tmp_path):
        """Test that sportec data includes ball_status and ball_poss_team_id in CDF output."""
        # Verify sportec has both flags
        assert (
            dataset_sportec.metadata.flags
        ...
        # Read and parse the tracking data
        with open(tracking_path) as f:
            frames = [json.loads(line) for line in f]

        assert len(frames) == 202

        # Check that ball_status is present in all frames
        for frame in frames:
            assert "ball_status" in frame
            assert isinstance(frame["ball_status"], bool)

        # Check that ball_poss_team_id has correct type when present
        for frame in frames:
            if "ball_poss_team_id" in frame:
                assert isinstance(frame["ball_poss_team_id"], str)

    def test_skillcorner_flags_in_output(self, dataset_skillcorner, tmp_path):
        """Test that skillcorner data includes ball_status and ball_poss_team_id in CDF output."""
        # Verify skillcorner has both flags
        assert (
            dataset_skillcorner.metadata.flags
