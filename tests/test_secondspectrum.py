...
class TestOnlyAliveParameter:
    ...
    def test_only_alive_default_false(self):
        """Test that only_alive defaults to False (includes all frames)."""
        df, _, _, _ = secondspectrum.load_tracking(RAW_DATA_PATH, META_DATA_PATH)

        # Should include alive frames
        alive_rows = df.filter(pl.col("ball_state") == "alive")
        assert alive_rows.height > 0
