...
class TestSecondSpectrumArrowInputContract:
    ...
    def test_accepts_buffered_reader(self, raw_bytes, meta_bytes, tmp_path):
        assert_arrow_accepts_buffered_reader(_ss_load_arrow, tmp_path, raw_bytes, meta_bytes)

    def test_accepts_gzip_stream(self, raw_bytes, meta_bytes):
        assert_arrow_accepts_gzip_stream(_ss_load_arrow, raw_bytes, meta_bytes)


class TestSecondSpectrumArrowIncludeGameId:
    ...
    def test_false_omits_game_id(self, raw_bytes, meta_bytes):
        ds = secondspectrum.load_tracking(raw_bytes, meta_bytes, engine="arrow", include_game_id=False)
        assert "game_id" not in ds.tracking.column_names

    def test_str_overrides_game_id(self, raw_bytes, meta_bytes):
        ds = secondspectrum.load_tracking(raw_bytes, meta_bytes, engine="arrow", include_game_id="custom_123")
